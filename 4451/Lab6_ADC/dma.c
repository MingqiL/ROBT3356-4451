...
void configDMAx_ADC12(unsigned int sourceAddr, unsigned int *destAddr, unsigned int arraySize)
{
    ...
    DMA1SA = sourceAddr;     // source address
    DMA1DA = destAddr; // destination address

    DMACTL0 |= DMA1TSEL__ADC12IFG;       // DMA1 trigger input
    DMA1SZ = arraySize;   // DMA0 transfer size (1 word)

    // single transfer, source increment, interrupt enable
    DMA1CTL = DMADT_0 | DMADSTINCR_3 | DMAEN;// | DMAIE;
}
