...
static unsigned char previousCol=0;
/************************************************************************************
* Function: nokLcdInit
* -
* argument:
...
void nokLcdInit(void) {
    LCDDir |= POW + LED + DAT_CMD + RST + SCE;
    LCDPort |= POW + SCE + RST + LED;
    LCDPort &= ~SCE;
...
void nokLcdWrite(char lcdByte, char cmdType) {
    ...
    if(cmdType == DC_DAT)   LCDPort |= DAT_CMD;   // data mode
    else                    LCDPort &= ~DAT_CMD;  // command mode

    LCDPort &= ~SCE;          // activate the SCE. the chip select
    usciB0SpiPutChar(lcdByte);// transmit lcdByte with spiTxByte
...
void nokLcdClear(){
    ...
    for (j=0 ; j<(LCD_MAX_COL) ; j++ ){
        for ( i=0 ; i<LCD_MAX_ROW / LCD_ROW_IN_BANK ; i++ ){
            nokLcdWrite(LCD_SET_XRAM | j, DC_CMD);
...
void updateLCD(float axisPos)
{
    nokLcdDrawLinearAxisLine(previousCol, 0);   // clear last line
    previousCol = (unsigned char)(axisPos * AXIS_MAX_COL);
    nokLcdDrawLinearAxisLine(previousCol, 1);   // draw new line
}

int nokLcdDrawLinearAxisLine(unsigned char x_AL, unsigned char mode)
{
    // verify pixel position is valid
    if (x_AL < LCD_MAX_COL) {
        ...
        // set mode
        // set the x and y RAM address  corresponding to the desired (x,bank) location. this is a command DC_CMD
        nokLcdWrite(LCD_SET_XRAM | x_AL, DC_CMD);
        nokLcdWrite(LCD_SET_YRAM | 2, DC_CMD);

        if (mode)
        {
            // update the pixel being set in currentPixelDisplay array
            currentPixelDisplay[x_AL][2] |= 0xFF;
        }
        else
        {
            currentPixelDisplay[x_AL][2] &= ~0xFF;
        ...
        nokLcdWrite(currentPixelDisplay[x_AL][2], DC_DAT); // write the data. this is DATA DC_DAT



        nokLcdWrite(LCD_SET_YRAM | 3, DC_CMD);
        if (mode)
        {
            currentPixelDisplay[x_AL][3] |= 0xFF;
        }
        else{
            currentPixelDisplay[x_AL][3] &= ~0xFE;
        }
        nokLcdWrite(currentPixelDisplay[x_AL][3], DC_DAT);
