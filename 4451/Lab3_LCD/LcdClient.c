#include <msp430.h> 
#include <nok5110LCD.h>
#include <string.h>
#include <Uart_lab.h>
#include <CmdInterpreter.h>
#include <usciSpi.h>

/**
 * main.c
 */

char rxString[BUFF_SZ];
char rxSpiBuff[BUFF_SZ];
unsigned char i=0;

int main(void)
{
	WDTCTL = WDTPW | WDTHOLD;	// stop watchdog timer
	unsigned char sclkMode[2] = {0,0};  // sclkMode<1> = UCCKPH, sclkMode<2> = UCCKPL
	int buffLen,  cmdNum = -1;

	// P2.7: RST
	// P4.0: SCE
	// P4.1: SIMO
	// P4.2: D/C
	// P4.3: SMCLK
	// P6.0: SS

	ucsiA1UartInit();   // initiate Uart
	usciB1SpiInit(MASTER, 3, sclkMode, 0);  // initiate SPI
	nokLcdInit();   // initiate LCD
	cmdsinit();     // initiate cmds

//	__bis_SR_register(GIE);
//	UCB1IE |= UCRXIE;
//	       | UCTXIE;
//	UCA1IE |= UCRXIE;
//	       | UCTXIE;

	//forFun();
	//nokLcdClear();

	do{
	    usciA1UartGets(rxString);
	    buffLen = strlen(rxString);
	    cmdNum = parseCmd(rxString);
	    executeCMD(cmdNum);

	    //usciB1SpiTxBufferSS(rxString,buffLen-1);
	    //drawCheck = nokLcdDrawScrnLine(5,5,0);
	    //drawCheck = nokLcdDrawLine(5,5,40,1);

	}while(1);


return 0;

}

#pragma vector = USCI_A1_VECTOR
__interrupt void USCI_A1_ISR(void) {
  switch(__even_in_range(UCA1IV,4))
  {
  case 0:break;
  case 2: //RX int
      while (!(UCA1IFG & UCTXIFG)); // is this efficient ?
      usciB1SpiPutChar(UCA1RXBUF);   // write UART RXBUF to SPI TXBUF

    break;
  case 4:break; //TX int
  default: break;
  }
}

#pragma vector=USCI_B1_VECTOR
__interrupt void usciB1SpiIsr(void){
    switch(__even_in_range(UCB1IV,4))
    {
    case 0:break;
    case 2: //RX int
        rxSpiBuff[i++] = UCB1RXBUF;
        if (i == BUFF_SZ) i=0;
      break;
    case 4:break; //TX int
    default: break;
}
}
